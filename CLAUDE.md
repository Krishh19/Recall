...
Copy-pasteable prompts for Claude Code, Cursor, Windsurf, or any AI IDE, scoped to one feature at a time and matching the tech stack and roadmap from the blueprint doc. Paste Section 1 once as standing project context; work through Section 2 in order, reviewing the output before moving to the next prompt.
...
Save this as `CLAUDE.md` (Claude Code) or `.cursorrules` (Cursor) so every prompt after it inherits the context without you re-explaining the stack each time.
